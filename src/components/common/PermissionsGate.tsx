...
export default function PermissionsGate(props: PermissionsGateProps) {
    const { children, scopes, posterKey, RenderError } = props;
    const { role } = useRole();
    const permissions = PERMISSIONS[role];
    const Forum = useForum();
    const wallet = Forum.wallet;
    const permissionGranted = hasPermission({ permissions, scopes }) || wallet.publicKey?.toBase58() === posterKey?.toBase58();
